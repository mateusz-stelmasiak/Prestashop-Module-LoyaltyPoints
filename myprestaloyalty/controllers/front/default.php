...
class myprestaloyaltyDefaultModuleFrontController extends ModuleFrontController
{
    ...
    public function __construct()
    {
        ...
        include_once(_PS_CLASS_DIR_.'Link.php');
        include_once(_PS_CLASS_DIR_.'CartRule.php');
    ...
    public function processTransformPoints()
    {
        ...
        if ($customer_points > 0) {
            ...
            if(empty(Context::getContext()->cart->getProducts())){
                //redirect to panel
                Tools::redirect($this->context->link->getModuleLink('myprestaloyalty', 'default', array('process' => 'summary')));
                return;
            }

            //automatically add voucher to cart
            Context::getContext()->cart->addCartRule($id_cart_rule);

            //redirect to cart
            $link = new Link();
            Tools::redirect($link->getPageLink('cart', null, null, ['action' => 'show']));
        }
