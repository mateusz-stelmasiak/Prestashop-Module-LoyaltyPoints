...
class LoyaltyModule extends ObjectModel
{
    ...
    public static function getAvailablePointsByIdCustomer($id_customer){
        include_once(dirname(__FILE__) . '/LoyaltyStateModule.php');
        
        $toUseState = LoyaltyStateModule::getValidationId();
        $nonAwardedState = LoyaltyStateModule::getNoneAwardId();
        $query = '
		SELECT SUM(f.points) points FROM `'._DB_PREFIX_.'loyalty` f
		WHERE f.id_customer = '.(int)($id_customer).'
		AND f.id_loyalty_state LIKE '.$toUseState.' OR f.id_loyalty_state LIKE '.$nonAwardedState;
